...
export default function BottomDisplayMenu({loadedAnimationName, randomize}){
  ...
  const [verticalOffset, setVerticalOffset] = useState(characterManager?.getCharacterVerticalOffset?.() ?? 0);
  ...
  return (
        <div className={styles["Container"]}>
          ...
          <button
            ...
            aria-label={isExpanded ? "Collapse pose controls" : "Expand pose controls"}
          >
            <span className={styles["expandArrow"]} />
          </button>
          <div className={`${styles["ContainerPosition"]} ${isExpanded ? styles["expandedPanel"] : ""}`}>
            <div className={styles["topLine"]} />

            <div className={styles["verticalControl"]}>
              <label htmlFor="character-vertical-offset">Character height</label>
              <input
                id="character-vertical-offset"
                type="range"
                min="-1.5"
                max="1.5"
                step="0.01"
