...
export function getMixamoAnimation( animations, model, vrm ) {
    ...
    const motionHips = model.getObjectByName('mixamorigHips') || model.getObjectByName('mixamorig:Hips');
    if (!motionHips) {
    ...
    clip.tracks.forEach( ( origTrack ) => {
        ...
        const normalizedRigName = mixamoRigName.replace(/^mixamorig:/, 'mixamorig');
        const vrmBoneName = VRMRigMapMixamo[ normalizedRigName ];
        //const vrmNodeName = vrm.humanoid?.getNormalizedBoneNode( vrmBoneName )?.name;
        const vrmNodeName = vrmBoneName;
        // console.log("name", vrmNodeName, vrmBoneName);
            const mixamoRigNode = model.getObjectByName(mixamoRigName) || model.getObjectByName(normalizedRigName);
