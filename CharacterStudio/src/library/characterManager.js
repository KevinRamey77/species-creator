...
const DEFAULT_CHARACTER_OFFSET = [0, 0.12, 0];
...
export class CharacterManager {
  ...
  screenshotManager
    constructor(options){
      this._start(options);
      this.characterVerticalOffset = 0;
    }

    setCharacterVerticalOffset(offsetY = 0) {
      const nextValue = Number.isFinite(offsetY) ? offsetY : 0;
      this.characterVerticalOffset = nextValue;

      if (this.characterModel?.position) {
        this.characterModel.position.y = nextValue;
      }

      if (this.rootModel?.position) {
        this.rootModel.position.y = 0;
      }
    ...
    getCharacterVerticalOffset() {
      return this.characterVerticalOffset;
    }
     ...
     _applySpringBoneColliders(vrm) {
      ...
      const getColliders = ()=>{
        ...
        Object.entries(this.avatar).map(([_, entry]) => {
          if (!entry?.vrm?.springBoneManager) {
            return;
          }
    ...
    _positionModel(model){
      ...
      model.scene.position.x = defaultOffset[0] ?? 0;
      model.scene.position.z = defaultOffset[2] ?? 0;
      model.scene.position.y = defaultOffset[1] ?? 0;

      if (model?.scene?.position) {
        model.scene.position.set(defaultOffset[0] ?? 0, defaultOffset[1] ?? 0, defaultOffset[2] ?? 0)
      }

      if (model?.userData?.vrm?.meta?.metaVersion === '1' || model?.userData?.vrm?.meta?.metaVersion === '0') {
        model.scene.position.y = defaultOffset[1] ?? 0;
      }
    }
